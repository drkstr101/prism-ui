...
import { TODO_APP_URL } from './todo-list-app/test-data';
...
describe('Multi-actor scenarios', () => {
  ...
  describe('Todo List App', () => {
    it('supports multiple actors using separate browsers', async ({ actorCalled }) => {
      await actorCalled('Alice').attemptsTo(startWithAListContaining('Feed the cat'));

      await actorCalled('Bob').attemptsTo(startWithAListContaining('Walk the dog'));

      await actorCalled('Alice').attemptsTo(Ensure.that(itemNames(), equals(['Feed the cat'])));

      await actorCalled('Bob').attemptsTo(Ensure.that(itemNames(), equals(['Walk the dog'])));
    });
  });
});
