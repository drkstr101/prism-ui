...
export const isDisplayedAsCompleted = () =>
  Expectation.to<PageElement>('get displayed as completed').soThatActual(
    hasCssClass('completed'),
  );

export const isDisplayedAsOutstanding = () =>
  Expectation.to<PageElement>('get displayed as outstanding').soThatActual(
    not(hasCssClass('completed')),
  );
