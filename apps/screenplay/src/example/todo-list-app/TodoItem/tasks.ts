...
export const recordItem = (name: Answerable<string>): Task =>
  Task.where(
    d`#actor records an item called ${name}`,
...
export const remove = (item: Answerable<PageElement>): Task =>
  Task.where(d`#actor removes ${item}`, Hover.over(item), Click.on(destroyButton().of(item)));
...
export const rename = (item: Answerable<PageElement>) => ({
  to: (newName: Answerable<string>) =>
    Task.where(
      d`#actor renames ${item} to ${newName}`,
      edit(item),
...
export const markAsCompleted = (item: Answerable<PageElement>): Task =>
  Task.where(
    d`#actor marks ${item} as completed`,
    Check.whether(item, isDisplayedAsOutstanding()).andIfSo(
      toggle(item),
...
export const markAsOutstanding = (item: Answerable<PageElement>): Task =>
  Task.where(
    ...
    Check.whether(item, isDisplayedAsCompleted()).andIfSo(
      toggle(item),
...
export const toggle = (item: Answerable<PageElement>): Task =>
  Task.where(
    d`#actor toggles the completion status of ${item}`,
    Click.on(toggleButton().of(item)),
  );
